        ...
        content: SingleChildScrollView(
          child: SizedBox(
            ...
            child: _WebDatePicker(
              ...
              withoutOkButtons: withoutOkButtons,
...
class _WebDatePickerState extends State<_WebDatePicker> {
      ...
      if (_viewStartDate.month == date.month) {
        ...
        final color =
            isEnabled
                  ...
                  : widget.selectedDayColor?.withValues(alpha: 0.5) ?? theme.colorScheme.primary
        ...
        final cellTextStyle = isSelected
            ...
            : textStyle?.copyWith(
                color: isWeekend && widget.weekendDaysColor != null
                    ? widget.weekendDaysColor?.withValues(alpha: 0.5)
                    : theme.disabledColor,
              );

        final isHovered =
            widget.enableDateRangeSelection &&
  ...
  List<Widget> _buildMonthsOfYearCells(ThemeData theme) {
    ...
    for (int i = 1; i <= 12; i++) {
      ...
      final color = isEnabled ? theme.colorScheme.primary : theme.colorScheme.primary.withValues(alpha: 0.5);
      ...
      children.add(Padding(padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 16.0), child: child));
    }
    return children;
  }
  ...
  List<Widget> _buildYearsCells(ThemeData theme) {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      children.add(Padding(padding: const EdgeInsets.symmetric(horizontal: 8.0, vertical: 8.0), child: child));
    }
    return children;
  }
  ...
  List<Widget> _buildYearsOfCenturyCells(ThemeData theme) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      final isEnabled =
          (widget.firstDate.year <= date.year || (widget.firstDate.year - date.year) <= 20) &&
      ...
      children.add(Padding(padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 8.0), child: child));
    }
    return children;
  }
            ...
            Row(
              children: [
                ...
                if (!widget.withoutActionButtons)
                  _iconWidget(Icons.restart_alt, tooltip: localizations.backButtonTooltip, onTap: _onResetState),
                if (!widget.withoutActionButtons) const SizedBox(width: 4.0),

                /// Today
                if (!widget.withoutActionButtons)
                  _iconWidget(Icons.today, tooltip: localizations.currentDateLabel, onTap: _onStartDateChanged),
                const Spacer(),
