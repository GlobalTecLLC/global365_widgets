...
class _TimeZoneDropdownState extends State<TimeZoneDropdown> {
  @override
  void initState() {
    gLogger("List is ${BusinessProfileController.to.timezoneList}");
  ...
  Future<void> fetchData() async {
    if (BusinessProfileController.to.timezoneList.isEmpty) {
      setState(() {
        isLoading = false;
      });
      ...
      APIsCallGet.getData("Companies/GetConfigs?Type=sc").then((response) {
        ...
        if (response.statusCode == 200) {
          ...
          List parsed = payLoad['payload']['timeZones'];
          ...
          setState(() {
            listItems = tempNew;
            BusinessProfileController.to.timezoneList.value = tempNew;
          });
      ...
      final parsed = BusinessProfileController.to.timezoneList;
      List tempNew = isNotHistory
          ? []
          : [
              {"value": "0", "label": "All"},
    ...
    return Column(
      ...
      children: [
        ...
        (isLoading)
            ...
            : globalSpinkitForLoaderswithBorder(height: widget.containerHeight ?? 40),
      ],
    );
  }
